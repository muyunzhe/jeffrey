...
def load_final_test_data(file_path):
    ...
    for i,line in enumerate(lines):
        ...
        pred_string = pred_string.strip().replace("\n","")
        pred_lists_result.append((nid, mthid, pubt_time, pred_string))
...
def load_data_predict(vocabulary_word2index, max_sentence_num, max_sentence_length, vocab_size, pred_lists_result):
    ...
    for i, item in enumerate(pred_lists_result):
        nid, mthid, pubt_time, pred_string = item
        ...
        while True:
            ...
            if sentences:
                ...
                try:
                    ...
                    #print one_sample
                    #lock.acquire()
                    final_list.append((nid, mthid, pubt_time, one_sample))
                except Exception as e:
                    traceback.print_exc()
                #finally:
                #    lock.release()
