...
import numpy as np
from data_util_pro_multiprocess import load_data, create_voabulary, load_final_test_data, load_data_predict
# from tflearn.data_utils import pad_sequences #to_categorical
import os
import codecs
from pro_HierarchicalAttention_model_vec import HierarchicalAttention
...
def main(_):
    ...
    pred_info_list=[]
    for item in test:
        nid, mthid, pubt_time, img_num, pred_string_length, one_sample = item
        pred_info_list.append((nid, mthid, pubt_time, img_num, pred_string_length))
   ...
   # 2.create session.
    ...
    with tf.Session(config=config) as sess:
        ...
        for start, end in zip(range(0, number_of_training_data, FLAGS.batch_size),range(FLAGS.batch_size, number_of_training_data+1, FLAGS.batch_size)):
            doc_reprs=sess.run(model.document_representation, feed_dict={model.input_x:testX[start:end],model.dropout_keep_prob:1}) #'shape of logits:', ( 1, 1999)
            ...
            #get_label_1prob_using_logits_batch(pred_info_sublist, logits, predict_out_file)
            get_doc_representation(pred_info_sublist, doc_reprs, predict_out_file)

            index=index+1
        predict_out_file.close()

def get_doc_representation(pred_info_list, doc_repr_batch, f_out):
    for i, doc_repr in enumerate(doc_repr_batch):
        #print len(doc_repr)
        nid, mthid, pubt_time, img_num, pred_string_length = pred_info_list[i] 
        f_out.write('\t'.join([nid, mthid, ' '.join([str(sub) for sub in list(doc_repr)])]) + '\n')
...
def get_label_1prob_using_logits_batch(pred_info_list, logits_batch, f_out):
    for i, logits in enumerate(logits_batch):
        ...
        nid, mthid, pubt_time, img_num, pred_string_length = pred_info_list[i]
        f_out.write('\t'.join([str(sub) for sub in [mthid, nid, pred_label, pred_prob1, softmax_prob1, img_num, pred_string_length, pubt_time]]) + '\n')
