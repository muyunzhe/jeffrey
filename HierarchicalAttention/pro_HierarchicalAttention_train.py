...
from data_util_pro import load_data, create_voabulary
...
tf.app.flags.DEFINE_string("train_data_path","demo_sample.data","path of traning data.") #O.K.train-zhihu4-only-title-all.txt-->training-data/test-zhihu4-only-title.txt--->'training-data/train-zhihu5-only-title-multilabel.txt'
tf.app.flags.DEFINE_string("word2vec_model_path","sgns.wiki.bigram-char","word2vec's vocabulary and vectors") #zhihu-word2vec.bin-100-->zhihu-word2vec-multilabel-minicount15.bin-100
...
def main(_):
    ...
    if 1==1:
        ...
        vocabulary_word2index, vocabulary_index2word, vocabulary_wordind2vec = \
            create_voabulary(word2vec_model_path=FLAGS.word2vec_model_path,name_scope="hierAtten") #simple='simple'
        vocab_size = len(vocabulary_word2index)
        print("han_model.vocab_size:",vocab_size)
        ...
        train, test = load_data(vocabulary_word2index, 
    ...
    with tf.Session(config=config) as sess:
        ...
        print FLAGS.num_classes,FLAGS.learning_rate,FLAGS.learning_rate,FLAGS.decay_steps,FLAGS.decay_steps,\
                FLAGS.max_sentence_num,FLAGS.max_sentence_length,vocab_size,FLAGS.embed_size,FLAGS.hidden_size,FLAGS.is_training,FLAGS.multi_label_flag
        ...
        if os.path.exists(FLAGS.ckpt_dir+"checkpoint"):
            ...
            if FLAGS.use_embedding: #load pre-trained word embedding
                ...
                print 'vocabulary_wordind2vec length:', len(vocabulary_wordind2vec)
                word_embedding_2dlist = [[]] * vocab_size
                ind_sets = set()
                for ind, vec in vocabulary_wordind2vec.items():
                    word_embedding_2dlist[ind] = vec
                    ind_sets.add(ind)
                print 'ind length:', len(ind_sets)
                ...
                #                                 initializer=word_embedding_final,
                #                                 trainable=True)
                ...
                if FLAGS.batch_size!=0 and (start%(FLAGS.validate_step*FLAGS.batch_size)==0): #(epoch % FLAGS.validate_every) or  if epoch % FLAGS.validate_every == 0:
                    ...
                    #print("HierAtten==>Epoch %d Validation Loss:%.3f\tValidation Accuracy: %.3f" % (epoch, eval_loss, eval_acc))
                        ...
                        if eval_loss<best_eval_loss:
                            ...
                            save_path = FLAGS.ckpt_dir + "/model_han.ckpt"
